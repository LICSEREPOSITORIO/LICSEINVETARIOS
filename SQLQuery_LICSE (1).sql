...
(
	...
	sol_fecha  DATE,
	usuario INT FOREIGN KEY REFERENCES  USUARIO(id_usuario),
	fecha_progra DATETIME,
	solicitante VARCHAR(60),
	sede INT FOREIGN KEY REFERENCES SEDE(id_sede),
	tecnico INT FOREIGN KEY REFERENCES TECNICO(id_Tecnico)
)
...
-- MOVIMIENTO SALIDA = M_Salida
 
	SELECT			dbo.SOLICITUD.id_solicitud AS Solicitud, dbo.ELEMENTO.elem_ref AS Referencia, dbo.ELEMENTO.elem_nom AS Nombre, dbo.ENTRADA_SOLICITUD.cant AS Cantidad, 
					dbo.SOLICITUD.fecha_progra AS [Fecha salida], dbo.SOLICITUD.solicitante, dbo.SEDE.sede_nombre AS Sede
	FROM			dbo.ELEMENTO INNER JOIN
                    dbo.ENTRADA ON dbo.ELEMENTO.id_elem = dbo.ENTRADA.elemento INNER JOIN
                    ...
                    dbo.SEDE ON dbo.SOLICITUD.sede = dbo.SEDE.id_sede
